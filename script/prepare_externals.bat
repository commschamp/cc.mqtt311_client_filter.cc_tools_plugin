...
rem CC_MQTT311_GENERATED_REPO - (Optional) Repository of the cc.mqtt311.generated
rem CC_MQTT311_GENERATED_TAG - (Optional) Tag of the cc.mqtt311.generated
rem CC_MQTT311_LIBS_REPO - (Optional) Repository of the cc.mqtt311.libs
rem CC_MQTT311_LIBS_TAG - (Optional) Tag of the cc.mqtt311.libs
...
if [%CC_MQTT311_GENERATED_REPO%] == [] set CC_MQTT311_GENERATED_REPO="https://github.com/commschamp/cc.mqtt311.generated.git"

if [%CC_MQTT311_GENERATED_TAG%] == [] set CC_MQTT311_GENERATED_TAG="master"

if [%CC_MQTT311_LIBS_REPO%] == [] set CC_MQTT311_LIBS_REPO="https://github.com/commschamp/cc.mqtt311.libs.git"

if [%CC_MQTT311_LIBS_TAG%] == [] set CC_MQTT311_LIBS_TAG="master"
...
set CC_MQTT311_GENERATED_SRC_DIR=%EXTERNALS_DIR%/cc.mqtt311.generated
set CC_MQTT311_GENERATED_BUILD_DIR=%BUILD_DIR%/externals/cc.mqtt311.generated/build
set CC_MQTT311_GENERATED_INSTALL_DIR=%CC_MQTT311_GENERATED_BUILD_DIR%/install
if NOT [%COMMON_INSTALL_DIR%] == [] set CC_MQTT311_GENERATED_INSTALL_DIR=%COMMON_INSTALL_DIR%

set CC_MQTT311_LIBS_SRC_DIR=%EXTERNALS_DIR%/cc.mqtt311.libs
set CC_MQTT311_LIBS_BUILD_DIR=%BUILD_DIR%/externals/cc.mqtt311.libs/build
set CC_MQTT311_LIBS_INSTALL_DIR=%CC_MQTT311_LIBS_BUILD_DIR%/install
if NOT [%COMMON_INSTALL_DIR%] == [] set CC_MQTT311_LIBS_INSTALL_DIR=%COMMON_INSTALL_DIR%
...
if exist %CC_MQTT311_GENERATED_SRC_DIR%/.git (
    echo "Updating cc.mqtt311.generated..."
    cd %CC_MQTT311_GENERATED_SRC_DIR%
    git fetch --all
    git checkout .
    git checkout %CC_MQTT311_GENERATED_TAG%
    git pull --all
) else (
    echo "Cloning cc.mqtt311.generated ..."
    git clone -b %CC_MQTT311_GENERATED_TAG% %CC_MQTT311_GENERATED_REPO% %CC_MQTT311_GENERATED_SRC_DIR%
    if %errorlevel% neq 0 exit /b %errorlevel%
)

echo "Building cc.mqtt311.generated ..."
mkdir "%CC_MQTT311_GENERATED_BUILD_DIR%"
cd %CC_MQTT311_GENERATED_BUILD_DIR%
cmake %GENERATOR_PARAM% %PLATFORM_PARAM% -S %CC_MQTT311_GENERATED_SRC_DIR% -B %CC_MQTT311_GENERATED_BUILD_DIR% -DCMAKE_INSTALL_PREFIX=%CC_MQTT311_GENERATED_INSTALL_DIR% -DCMAKE_BUILD_TYPE=%COMMON_BUILD_TYPE% -DOPT_REQUIRE_COMMS_LIB=OFF -DCMAKE_PREFIX_PATH=%COMMS_INSTALL_DIR% -DCMAKE_CXX_STANDARD=%COMMON_CXX_STANDARD%
if %errorlevel% neq 0 exit /b %errorlevel%
cmake --build %CC_MQTT311_GENERATED_BUILD_DIR% --config %COMMON_BUILD_TYPE% --target install
if %errorlevel% neq 0 exit /b %errorlevel%

rem ----------------------------------------------------

if exist %CC_MQTT311_LIBS_SRC_DIR%/.git (
    echo "Updating cc.mqtt311.libs..."
    cd %CC_MQTT311_LIBS_SRC_DIR%
    git fetch --all
    git checkout .
    git checkout %CC_MQTT311_LIBSD_TAG%
    git pull --all
) else (
    echo "Cloning cc.mqtt311.libs ..."
    git clone -b %CC_MQTT311_LIBS_TAG% %CC_MQTT311_LIBS_REPO% %CC_MQTT311_LIBS_SRC_DIR%
    if %errorlevel% neq 0 exit /b %errorlevel%
)
    
echo "Building cc.mqtt311.libs ..."
mkdir "%CC_MQTT311_LIBS_BUILD_DIR%"
cd %CC_MQTT311_LIBS_BUILD_DIR%
cmake %GENERATOR_PARAM% %PLATFORM_PARAM% -S %CC_MQTT311_LIBS_SRC_DIR% -B %CC_MQTT311_LIBS_BUILD_DIR% -DCMAKE_INSTALL_PREFIX=%CC_MQTT311_LIBS_INSTALL_DIR% -DCMAKE_BUILD_TYPE=%COMMON_BUILD_TYPE% -DCMAKE_PREFIX_PATH=%COMMS_INSTALL_DIR%;%CC_MQTT311_GENERATED_INSTALL_DIR% -DCMAKE_CXX_STANDARD=%COMMON_CXX_STANDARD% -DCC_MQTT311_CLIENT_LIB_FORCE_PIC=ON -DCC_MQTT311_CLIENT_APPS=OFF
if %errorlevel% neq 0 exit /b %errorlevel%
cmake --build %CC_MQTT311_LIBS_BUILD_DIR% --config %COMMON_BUILD_TYPE% --target install
if %errorlevel% neq 0 exit /b %errorlevel%
